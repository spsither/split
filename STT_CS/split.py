...
if __name__ == "__main__":
    stt_files = [filename for filename in os.listdir("full_audio") if filename.startswith("STT_CS") and os.path.isfile(os.path.join("full_audio", filename))]
    print(stt_files)
    for stt_file in stt_files:
        print(stt_file)
        stt_file = stt_file.split(".")[0]
        split_audio(audio_file=f"./full_audio/{stt_file}.mp3", output_folder=stt_file)
